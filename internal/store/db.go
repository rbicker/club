...
func NewMongoConnection(secret string, opts ...func(*MGO) error) (*MGO, error) {
	...
	// handle secret
	if m.secret == "" {
		m.infoLogger.Printf("because no secret was given, a random string will be used")
		m.infoLogger.Printf("this means pagination tokens won't survive a restart of the application")
		m.secret = utils.RandomString(20)
	}
	h := md5.New()
...
// WithInfoLogger sets the info logger.
// By default, log entries will be written to stdout.
func WithInfoLogger(logger *log.Logger) func(*MGO) error {
	return func(m *MGO) error {
		m.infoLogger = logger
		return nil
	}
...
func WithErrorLogger(logger *log.Logger) func(*MGO) error {
	return func(m *MGO) error {
		m.infoLogger = logger
		return nil
	}
